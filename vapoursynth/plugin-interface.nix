...
take_plugins_asis: plugins: let
  ...
  finalPlugins = if take_plugins_asis then plugins else deepPlugins;
  ...
  pluginLoader = let
    source = writeText "vapoursynth-nix-plugins.c" ''
      ...
      ${lib.concatMapStringsSep "" (path: "load(data, \"${path}/lib/vapoursynth\");") finalPlugins}
      }
    '';
  in
